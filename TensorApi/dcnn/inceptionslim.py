...
class Inception:
    ...
    def classify(self, image_path=None):
       ...
       # probabilities = tf.nn.softmax(logits)
        ...
        tf.train.import_meta_graph("C:\\tmp\\model-meta\\raspi-model.meta")
        probabilities = self.session.run(tf.global_variables_initializer(), feed_dict={"DecodeJpeg/contents:0": image})
